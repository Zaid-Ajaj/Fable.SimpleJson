...
type test =
    ...
    static member inline areEqual a b = Expect.equal a b "They are equal"
    ...
    static member unexpected (x: obj) = Expect.isTrue false (Json.stringify x)
...
let everyTest =
  testList "Nagareyama tests" [
    ...
    // Wait for Fable 3 to be stable and Fable 2.x to obsolete
    ...
    testCase "Record with char can be converted" <| fun _ ->
        let input : RecordWithChar = { Char = '*' }
        input
        |> Json.serialize
